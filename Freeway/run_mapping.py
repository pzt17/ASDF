...
for frame_idx in range(1, num_frames + 1):
    ...
    if init_cnt >= 7:
    	...
    	if sum_mark >= 5:
    		...
    		'''
    		if random.randrange(100)%10 == 0:
    			true_state  = true_state + 5.0*random.randrange(-1,2,2)
    			replay_buffer.push(true_state, action, reward, next_state, done)
    			prew_cnt += 1
    			chk = 1
    		else:
    			replay_buffer.push(true_state, action, reward, next_state, done)

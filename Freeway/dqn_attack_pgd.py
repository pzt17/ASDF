...
def compute_loss(batch_size):
    ...
    next_q_value     = next_q_values.gather(1, action.unsqueeze(1)).squeeze(1)
    #print(next_q_value)
...
def pgd_attack( images, action, alpha= 5.0) : 
    ...
    for i in range(10):
    	...
    	Jx = states_loss[0]
    	images_t = images + alpha*Jx
    	delta = torch.clip(images_t - base_images, -1.0*alpha, 1.0*alpha)
    	images = base_images + delta
...
for frame_idx in range(1, num_frames + 1):
    ...
    adv_state = state
    if len(replay_buffer) >= replay_initial and random.randrange(100)%1 == 0:
      	adv_state = pgd_attack(state,action)
